...
function UserListRow(props) {
    ...
    const handleClick = () => {
        Axios.all([
            Axios.delete("http://54.204.74.125:4000/eventRoute/delete-user/" + _id)
            .then((res) => {
                if (res.status === 200) {
            ...
            .then((eventResponse) => {
                if(eventResponse.status === 200){
                    ...
                    for(let i = 0; i < collectedEvents.length; i++){
                        ...
                        Axios.put("http://54.204.74.125:4000/eventRoute/update-event/" + collectedEvents[i]._id, eventData)
                        .then((updateResponse) => {
